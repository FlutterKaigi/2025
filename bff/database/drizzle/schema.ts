import { pgTable, pgSchema, index, foreignKey, uuid, timestamp, text, inet, uniqueIndex, check, boolean, json, varchar, jsonb, smallint, bigserial, unique, integer, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const basicPlanType = pgEnum("basic_plan_type", ['platinum', 'gold', 'silver', 'bronze'])
export const companySponsorType = pgEnum("company_sponsor_type", ['basic', 'community', 'tool', 'none'])
export const optionPlanType = pgEnum("option_plan_type", ['naming_rights_hall', 'naming_rights_room', 'nameplate', 'lunch', 'scholarship'])
export const role = pgEnum("role", ['admin', 'staff', 'sponsor', 'speaker', 'viewer', 'attendee'])
export const snsType = pgEnum("sns_type", ['github', 'x', 'discord', 'medium', 'qiita', 'zenn', 'note', 'other'])
...
export const ticketPurchases = pgTable("ticket_purchases", {
	...
	unique("ticket_purchases_stripe_payment_intent_id_key").on(table.stripePaymentIntentId),
]);
...
export const news = pgTable("news", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "news_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	title: text().notNull(),
	url: text(),
...
export const basicSponsorCompanies = pgTable("basic_sponsor_companies", {
	id: smallint().primaryKey().generatedAlwaysAsIdentity({ name: "basic_sponsor_companies_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 32767, cache: 1 }),
	...
	unique("basic_sponsor_companies_sponsor_company_id_key").on(table.sponsorCompanyId),
]);
...
export const sponsorCompanies = pgTable("sponsor_companies", {
	id: smallint().primaryKey().generatedAlwaysAsIdentity({ name: "sponsor_companies_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 32767, cache: 1 }),
	companyId: smallint("company_id"),
...
export const sponsorCompanyOptions = pgTable("sponsor_company_options", {
	id: smallint().primaryKey().generatedAlwaysAsIdentity({ name: "sponsor_company_options_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 32767, cache: 1 }),
	sponsorCompanyId: smallint("sponsor_company_id"),
	...
	unique("sponsor_company_options_sponsor_company_id_option_plan_type_key").on(table.sponsorCompanyId, table.optionPlanType),
]);
...
export const userSnsLinks = pgTable("user_sns_links", {
	id: uuid().default(sql`uuid_generate_v7()`).primaryKey().notNull(),
	userId: uuid("user_id").notNull(),
	...
	unique("user_sns_links_user_id_sns_type_key").on(table.userId, table.snsType),
	check("user_sns_links_value_check", sql`char_length(TRIM(BOTH FROM value)) > 0`),
]);
