...
export const ticketCheckoutSessions = pgTable(
  "ticket_checkout_sessions",
  {
    ...
    expiresAt: timestamp("expires_at", { withTimezone: true, mode: "string" })
      .default(sql`(now() + '00:30:00'::interval)`)
      .notNull(),
    createdAt: timestamp("created_at", { mode: "string" })
      .defaultNow()
      .notNull(),
  ...
  (table) => [
    ...
    index("idx_ticket_checkout_sessions_stripe_payment_intent_id").using(
      "btree",
      table.stripePaymentIntentId.asc().nullsLast().op("text_ops")
    ),
    ...
    unique("ticket_checkout_sessions_user_id_ticket_type_id_status_key").on(
      table.userId,
      table.ticketTypeId,
      table.status
    ),
