alter table "public"."ticket_options" add column "max_quantity" integer;
...
BEGIN
  ...
  INSERT INTO public.ticket_checkout_sessions (
    ...
    'pending'::public.ticket_checkout_status,
...
CREATE OR REPLACE FUNCTION public.get_ticket_count(ticket_type_id text, OUT sold_count integer, OUT reserved_count integer)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
BEGIN
  ...
  WHERE ticket_type_id = get_ticket_count.ticket_type_id;
...
CREATE OR REPLACE FUNCTION public.get_ticket_option_count(ticket_option_id text, OUT sold_count integer, OUT reserved_count integer)
 RETURNS record
 LANGUAGE plpgsql
AS $function$
BEGIN
